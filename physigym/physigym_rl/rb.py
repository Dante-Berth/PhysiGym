...
class ReplayBuffer:
    ...
    def __init__(
        ...
        print(
            f"ReplayBuffer | state_dim={state_dim} | is_graph={is_graph} | "
        ...
        if self.is_graph:
            ...
            try:
                # ── Preallocate everything on device ──────────────
                # Pinned CPU memory → async copy to GPU when sampling
                pin = str(device) != "cpu"
                self.state = torch.empty(
                    (self.buffer_size, *state_dim), dtype=torch_dtype, pin_memory=pin
                ).to(device)
                self.next_state = torch.empty(
                    (self.buffer_size, *state_dim), dtype=torch_dtype, pin_memory=pin
                ).to(device)
                self.action = torch.empty(
                    (self.buffer_size, *action_dim), dtype=torch.float32, pin_memory=pin
                ).to(device)
                self.reward = torch.empty(
                    (self.buffer_size, 1), dtype=torch.float32, pin_memory=pin
                ).to(device)
                self.done = torch.empty(
                    (self.buffer_size, 1), dtype=torch.uint8, pin_memory=pin
                ).to(device)
                self.use_torch_tensors = True
            except (RuntimeError, TypeError) as e:
                print(
                    f"[ReplayBuffer] GPU prealloc failed ({e}), falling back to NumPy"
                )
    ...
    def add_batch(self, batch):
        ...
        if self.use_torch_tensors:
            # Stack into tensors once — avoids n individual copy_ calls
            s = torch.stack([torch.as_tensor(x) for x in states])
            a = torch.stack([torch.as_tensor(x) for x in actions])
            r = torch.tensor(rewards, dtype=torch.float32).unsqueeze(-1)
            ns = torch.stack([torch.as_tensor(x) for x in next_states])
            d = torch.tensor(dones, dtype=torch.uint8).unsqueeze(-1)
            ...
            if end <= self.buffer_size:
                ...
                self.action[: end % self.buffer_size].copy_(
                    a[first:], non_blocking=True
                )
                self.reward[self._ptr :].copy_(r[:first], non_blocking=True)
                self.reward[: end % self.buffer_size].copy_(
                    r[first:], non_blocking=True
                )
                self.next_state[self._ptr :].copy_(ns[:first], non_blocking=True)
                self.next_state[: end % self.buffer_size].copy_(
                    ns[first:], non_blocking=True
                )
    ...
    def _dict_to_pyg(self, s):
        return Data(
            ...
            edge_attr=torch.as_tensor(
                s["edges"], dtype=torch.float32, device=self.device
            ),
        )
        ...
        if self.use_torch_tensors:
            ...
            return {
                "state": torch.as_tensor(
                    self.state[idx_np], device=self.device
                ).float(),
                ...
                "next_state": torch.as_tensor(
                    self.next_state[idx_np], device=self.device
                ).float(),

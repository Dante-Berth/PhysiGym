...
def make_physigym_env(env_id: int, cfg: dict):
    ...
    def _init():
        ...
        if "settingcells" in local_model_cfg: del local_model_cfg["settingcells"]
        if "output_dir" in local_model_cfg: del local_model_cfg["output_dir"]
...
if __name__ == "__main__":
    ...
    while step < MAX_STEPS:
        ...
        for _rep in range(ACTION_REPEAT):
            ...
            acc_reward  += reward
            last_obs, last_info, last_term, last_trunc = obs, info, terminated, truncated
            step += 1
        ...
        if last_term or last_trunc:
            ...
            ep_steps    = 0
            prev_action = None
            deltas_x.clear(); deltas_y.clear(); deltas_r.clear()
            env.reset()
            env.generate_physicell_data = True

...
The $d_t$ term penalises drug use without conflating temporal dosing
strategy with total exposure.

\paragraph{Distributions.}
\label{sec:distributions}
Training initial conditions are sampled from a spatially correlated
\emph{network-field} process (see Appendix~\ref{app:ic} for details),
yielding irregular clustered layouts with $\approx128$ tumor cells,
32 T-cells, and 16 macrophages.
Two held-out layouts serve as test distributions to measure
out-of-distribution generalisation: a \textbf{circular} layout, where
each cell type is placed in a compact elliptic cluster, and a
\textbf{rectangle} layout, where each type occupies a narrow vertical
strip of the domain.
Neither layout is seen during training.
Figure~\ref{fig:ic_distributions} shows one representative example of each.
...
\begin{figure}[h]
  ...
  \caption{Initial condition distributions.
    ...
    Tumor cells are shown in grey, T-cells in red, and macrophages in blue.}
...
chemical compounds (substrates). \physigym{} can expose all of them as
observations, from spaceless global statistics to full spatial images and
cell-interaction graphs.
In this work we compare six representative modes spanning scalars to images;
graph-based representations (Delaunay triangulation, $k$-NN graphs) and
more expressive structured scalar modes (K-means cluster descriptors,
relational nearest-neighbour features) are supported by the framework but
...
\paragraph{S3 (\Sthree{}, $N_c + 6N_c = 21$ dims).}
S1 plus per-type spatial statistics (6 values each):
presence flag, centroid $(x_\mu, y_\mu)$, spread $(x_\sigma, y_\sigma)$,
and distance to domain centre — all normalised to $[0,1]$.

\paragraph{S5 (\Sfive{}, $N_c + 6N_c + 6N_s = 39$ dims).}
S3 extended with per-substrate spatial statistics:
global mean, standard deviation, min, max, and concentration-weighted
centroid $(x_c, y_c)$.
...
there, then per-channel min–max normalised.

\paragraph{I1 (\Ione{}, $N_c\!\times\!64\!\times\!64$).}
One channel per cell type.

\paragraph{I2 (\Itwo{}, $(N_c\!+\!N_s)\!\times\!64\!\times\!64$).}
...
Appendix~\ref{app:episodes} shows two matched episodes (run\_000143 and
...
This appendix describes the procedure used to generate spatially structured
initial conditions for synthetic cellular networks. The method constructs
spatially correlated random fields that bias the placement of different cell
types while preserving stochasticity at both global and local scales.

\subsection*{Gaussian-Correlated Random Fields}

Let $\Omega \subset \mathbb{Z}^2$ denote a two-dimensional spatial domain of
size $N_x \times N_y$. A white noise field
$\eta(\mathbf{x}) \sim \mathcal{N}(0,1)$, $\mathbf{x} \in \Omega$,
is convolved with a Gaussian kernel
\[
  G_\sigma(\mathbf{r}) = \frac{1}{2\pi\sigma^2}
  \exp\!\left(-\frac{\|\mathbf{r}\|^2}{2\sigma^2}\right),
  \qquad \sigma = \frac{\ell_c}{\sqrt{2}},
\]
yielding the correlated field $F(\mathbf{x}) = (G_\sigma * \eta)(\mathbf{x})$,
where $\ell_c$ is the prescribed spatial correlation length.
Reflective boundary conditions are applied to avoid edge artifacts.

\subsection*{Balanced Fields with Local Fluctuations}

For each cell type $i$, a distinct field $F_i(\mathbf{x})$ is constructed by
adding a locally filtered noise component $\tilde{\xi}_i = (G_{\ell_c/3} *
\xi_i)(\mathbf{x})$ with small weight $\alpha$:
\[
  F_i(\mathbf{x}) = A\bigl[F(\mathbf{x}) + \alpha\,\tilde{\xi}_i(\mathbf{x})\bigr].
\]
Each field is then normalised and inverted:
\[
  \hat{F}_i(\mathbf{x}) =
  \frac{\max(F_i) - F_i(\mathbf{x})}{\max(F_i) - \min(F_i)},
  \quad \hat{F}_i \in [0,1].
\]

\subsection*{Probabilistic Cell Placement}

$N_i$ cell positions are sampled from the region
$\Omega_i = \{\mathbf{x} \mid \hat{F}_i(\mathbf{x}) > \theta_i\}$
with probability proportional to $\hat{F}_i$:
\[
  P(\mathbf{x}) = \frac{\hat{F}_i(\mathbf{x})}
  {\sum_{\mathbf{y}\in\Omega_i}\hat{F}_i(\mathbf{y})}.
\]
The procedure is repeated independently for each cell type.
Parameters used: $\ell_c = 45$, $\theta = 0.55$, approximately 128 tumor
cells, 32 T-cells, and 16 macrophages per episode.

\section{Neural Architecture}
\label{app:arch}

We employ a modular neural architecture following an actor-critic paradigm,
designed to accommodate heterogeneous observation modalities, including
image-based and vector-based state representations. A shared feature extraction
module encodes observations into a common latent representation, which is then
used by separate policy (actor) and value (critic) networks.

The feature extractor dynamically adapts to the observation type.
Image-based inputs are processed through a convolutional encoder;
vector observations are passed directly without transformation.
In all cases, the extracted features are flattened into a fixed-dimensional
latent vector that serves as the input to both actor and critic networks.

\subsection*{IMPALA-style Convolutional Network}

For image-based inputs we use an IMPALA-style convolutional
network~\citep{espeholt2018impala}. The architecture consists of a stack of
convolutional blocks. Each block comprises:
\begin{itemize}
  \item a $3\times3$ convolution,
  \item a $3\times3$ max-pooling operation with stride 2,
  \item two residual blocks with skip connections.
\end{itemize}
Each residual block applies two convolutional layers with Mish~\citep{misra2019mish}
nonlinearities and preserves the input through identity shortcuts.
This structure promotes stable gradient flow and enables deep feature
hierarchies.

\subsection*{Actor--Critic Networks}

Both actor and critic share the same backbone after feature extraction:
two fully connected layers of width 256 with Mish activations (critic)
and ReLU activations (actor).
...
\label{alg:async_sac}
\begin{algorithmic}[1]
\Require Simulator configuration $d_{\text{arg}}$, replay buffer size
         $|\mathcal{B}|$, learning rates, discount factor $\gamma$,
...
\State Initialize target critics $Q_{\phi_1'} \leftarrow Q_{\phi_1}$,
...
\While{total samples $< T$}
    ...
    \For{$N_{\text{loops}}$ gradient steps}
        ...
        $y = r + \gamma (1-d)\bigl[\min(Q_{\phi_1'}, Q_{\phi_2'})
             - \alpha \log \pi_\theta(a'|s')\bigr]$

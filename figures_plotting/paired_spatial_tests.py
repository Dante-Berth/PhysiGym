...
DIRLAB = {"rect2net": r"rect.\ $\to$ net.", "net2rect": r"net.\ $\to$ rect."}


def to_tex_bydir(df, path):
    """Per-direction table.  Return and gap average two opposite effects when
    directions are pooled, so the manuscript reports them split."""
    lines = [
        r"% Generated by paired_spatial_tests.py -- do not edit by hand.",
        r"\begin{tabular}{llcrc}", r"  \toprule",
        r"  \textbf{Metric} & \textbf{Direction} & \textbf{Sign consist.}"
        r" & \textbf{Mean $\Delta$ [95\% CI]} & $p$ \\", r"  \midrule",
    ]
    for i, (name, spec) in enumerate(METRICS.items()):
        if name == "train_return":
            continue
        for direction in ("rect2net", "net2rect"):
            sub = df[(df.metric == name) & (df.direction == direction)]
            if sub.empty:
                continue
            d = sub.delta.to_numpy()
            k, n, p = sign_test(d, spec["h1"])
            lo, hi = boot_ci(d)
            met = f"{LABEL[name]} ({PRED[spec['h1']]})" if direction == "rect2net" else ""
            lines.append(
                f"  {met} & {DIRLAB[direction]} & ${k}/{n}$ & "
                f"${d.mean():+.2f}$ [${lo:+.2f}$, ${hi:+.2f}$] & {fmt_p(p)} \\\\")
        if i < len(METRICS) - 1:
...
if __name__ == "__main__":
    ...
    to_tex_bydir(df, os.path.join(OUT, "tab_paired_spatial_bydir.tex"))
